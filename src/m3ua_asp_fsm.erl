...
handle_info({sctp, Socket, _PeerAddr, _PeerPort, {_AncData, Data}}, down,
		#statedata{socket = Socket} = StateData) when is_binary(Data) ->
	handle_down(m3ua_codec:m3ua(Data), StateData);
handle_info({sctp, Socket, _PeerAddr, _PeerPort, {_AncData, Data}}, inactive,
		#statedata{socket = Socket} = StateData) when is_binary(Data) ->
	handle_inactive(m3ua_codec:m3ua(Data), StateData);
handle_info({sctp, Socket, _PeerAddr, _PeerPort, {_AncData, Data}}, active,
		#statedata{socket = Socket} = StateData) when is_binary(Data) ->
	handle_active(m3ua_codec:m3ua(Data), StateData);
